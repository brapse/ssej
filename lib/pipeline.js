...
var affinity = function (flag) {
    return parallel.indexOf(flag) > -1 ? FOREIGN : NATIVE;
};
...
Worker.prototype.open = function (input) {
    ...
    for (var i = 0; i < numCPUs; i++) {
        ...
        Object.keys(process.env).forEach(function (k) {
            childEnvironment[k] = process.env[k];
        });

        childEnvironment['SSEJ_CHILD'] = true;

        var n = cp.fork(__dirname + '/../cli.js', defs.concat(p), {env: childEnvironment});
